...
### Added
- Technicians now have their own workspace to raise and follow up on their own tickets, with the same experience requesters have.
...
### Changed
- A technician's workspace now lists the tickets assigned to them rather than the tickets they raised themselves, so their queue reflects the work they are responsible for.
...
- Simplified the container build to a single Dockerfile, making the image easier to build and maintain; no user-facing behavior change.
- Dropped a third-party logging library in favor of the web framework's own logger, so application and framework messages now share a single log stream and one verbosity setting; one less dependency to build. No user-facing behavior change.
