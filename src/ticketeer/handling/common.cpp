...
std::size_t FetchTicketAttachmentMaxsize(sqlite3 *db) {
  sqlite3_stmt *stmt = nullptr;
  if (sqlite3_prepare_v2(db,
                         "SELECT ticket_attachment_maxsize FROM "
                         "helpdesk_setting WHERE name = 'default'",
                         -1, &stmt, nullptr) != SQLITE_OK) {
    LOG_DEBUG << "[handling] ticket attachment maxsize query error: "
              << sqlite3_errmsg(db);
    return 47185920;
  }
  std::size_t maxsize = 47185920;
  if (sqlite3_step(stmt) == SQLITE_ROW)
    maxsize = static_cast<std::size_t>(sqlite3_column_int64(stmt, 0));
  sqlite3_finalize(stmt);
  return maxsize;
}

std::size_t FetchTicketActivityAttachmentMaxsize(sqlite3 *db) {
  sqlite3_stmt *stmt = nullptr;
  if (sqlite3_prepare_v2(db,
                         "SELECT ticket_activity_attachment_maxsize FROM "
                         "helpdesk_setting WHERE name = 'default'",
                         -1, &stmt, nullptr) != SQLITE_OK) {
    LOG_DEBUG << "[handling] activity attachment maxsize query error: "
              << sqlite3_errmsg(db);
    return 15728640;
  }
  std::size_t maxsize = 15728640;
  if (sqlite3_step(stmt) == SQLITE_ROW)
    maxsize = static_cast<std::size_t>(sqlite3_column_int64(stmt, 0));
  sqlite3_finalize(stmt);
  return maxsize;
