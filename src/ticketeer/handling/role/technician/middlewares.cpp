#include "middlewares.hpp"

#include <sqlite3.h>
#include <trantor/utils/Logger.h>

#include "ticketeer/core/db.hpp"

namespace ticketeer::handling::role::technician::middlewares {

void RoleTechnicianRequired::invoke(const drogon::HttpRequestPtr &req,
                                    drogon::MiddlewareNextCallback &&nextCb,
                                    drogon::MiddlewareCallback &&mcb) {
  const auto token = req->getCookie("token");
  if (token.empty())
    return mcb(
        drogon::HttpResponse::newRedirectionResponse("/ticketeer/auth/signin"));

  sqlite3 *db = ticketeer::core::db::ConnectRO();
  if (!db)
    return Reject(mcb, "Database unavailable", drogon::k503ServiceUnavailable);

  sqlite3_stmt *stmt = nullptr;
  const char *sql = R"SQL(
SELECT p.id
  FROM helpdesk_profile p
  JOIN auth_session s
    ON s.user_id = p.user_id
 WHERE s.token = ?
   AND s.expires_at > datetime('now')
   AND p.role = 'technician'
 LIMIT 1
)SQL";

  bool allowed = false;
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK) {
    sqlite3_bind_text(stmt, 1, token.c_str(), -1, SQLITE_STATIC);
    allowed = sqlite3_step(stmt) == SQLITE_ROW;
    sqlite3_finalize(stmt);
  } else {
    LOG_DEBUG << "[technician] role middleware query error: "
              << sqlite3_errmsg(db);
  }

  sqlite3_close(db);

  if (!allowed)
    return mcb(
        drogon::HttpResponse::newRedirectionResponse("/ticketeer/auth/signin"));

  nextCb(std::move(mcb));
}

void RoleTechnicianRequired::Reject(const drogon::MiddlewareCallback &mcb,
                                    const char *msg,
                                    drogon::HttpStatusCode code) {
  auto resp = drogon::HttpResponse::newHttpResponse();
  resp->setStatusCode(code);
  resp->setBody(msg);
  mcb(resp);
}

} // namespace ticketeer::handling::role::technician::middlewares
