...
void AppendActivityPreviews(PGconn *pg, const char *log_prefix,
                            Json::Value &tickets) {
...
SELECT body,
 ...
 ORDER BY created_at DESC
    ...
    if (PQresultStatus(activities) != PGRES_TUPLES_OK) {
      LOG_DEBUG << log_prefix << " ticket activity preview query error: "
...
FetchTicketList(PGconn *pg, const char *log_prefix,
  ...
  if (PQresultStatus(tickets) != PGRES_TUPLES_OK) {
    LOG_DEBUG << log_prefix
              << " ticket list query error: " << PQresultErrorMessage(tickets);
    PQclear(tickets);
    return result;
  }
  ...
  PQclear(tickets);
  AppendActivityPreviews(pg, log_prefix, result);
  return result;
}

} // namespace

Json::Value FetchRequesterTicketList(PGconn *pg, const std::string &profile_id,
                                     const std::optional<std::string> &search) {
  return FetchTicketList(pg, "[requester]", &profile_id, search);
}

Json::Value
FetchSupervisorTicketList(PGconn *pg,
                          const std::optional<std::string> &search) {
  return FetchTicketList(pg, "[supervisor]", nullptr, search);
}
