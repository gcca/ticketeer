...
void Supervisor::ConfigTicketStatusDeletePost(
  ...
  if (!TicketStatusExists(db, ticket_status_id)) {
    sqlite3_close(db);
  ...
  if (!replacement_id.empty()) {
    if (!TicketStatusExists(db, replacement_id)) {
      sqlite3_close(db);
      return BadRequest(callback, "Replacement status not found",
                        drogon::k404NotFound);
    }
    if (!ReassignTicketStatus(db, ticket_status_id, replacement_id)) {
      sqlite3_close(db);
  ...
  if (!DeleteTicketStatus(db, ticket_status_id)) {
    sqlite3_close(db);
