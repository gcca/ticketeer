...
void Supervisor::TicketDueDatePost(const drogon::HttpRequestPtr &req,
  ...
  const auto ticket_trait = FetchTicketTrait(db, ticket_id);
  if (!ticket_trait) {
    sqlite3_close(db);
  ...
  if (!UpdateTicketDueDate(db, ticket_id, new_due_date)) {
    sqlite3_close(db);
    return BadRequest(callback, "Failed to update due date",
                      drogon::k400BadRequest);
  }

  InsertTicketActivity(db, ticket_id, profile->id, "due_date_changed",
