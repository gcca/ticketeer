...
VALUES (?, ?, ?, ?, ?, date('now', '-5 hours', '+' || ? || ' seconds'))
  ...
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
    LOG_DEBUG << "[supervisor] insert ticket error: " << sqlite3_errmsg(db);
    sqlite3_close(db);
  ...
  if (sqlite3_step(stmt) != SQLITE_DONE) {
    LOG_DEBUG << "[supervisor] insert ticket step error: "
              << sqlite3_errmsg(db);
  ...
  auto data = BuildTicketListData(db, TicketFilters{}, 1);
  sqlite3_close(db);
