...
void Supervisor::ConfigTicketPriorityCreatePost(const drogon::HttpRequestPtr &req,
                                                Callback &&callback) {
  ...
  if (!input)
    return BadRequest(callback, "Missing or invalid required fields: name, display_name");
  ...
  if (!InsertTicketPriority(logger, db, *input)) {
    sqlite3_close(db);
