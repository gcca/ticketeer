...
void Supervisor::ConfigTicketStatusListGet(const drogon::HttpRequestPtr &req,
                                           Callback &&callback) {
  ...
  const auto editing_id = req->getParameter("ts");
  const bool rendered = RenderTicketStatusConfig(callback, db, editing_id);
  sqlite3_close(db);
  if (!rendered)
