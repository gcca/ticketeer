...
[[nodiscard]] std::optional<Row> FetchDefaultSetting(sqlite3 *db) {
  ...
  if (sqlite3_step(stmt) == SQLITE_ROW)
    row = Row{{"default_status_id", ColumnText(stmt, 0)},
              ...
              {"ticket_attachment_maxsize", ColumnText(stmt, 8)},
              {"ticket_activity_attachment_maxsize", ColumnText(stmt, 9)}};
  sqlite3_finalize(stmt);
  return row;
}
...
bool UpsertDefaultSetting(sqlite3 *db, const std::string &default_status_id,
                          ...
                          int ticket_due_delta, int ticket_attachment_maxsize,
                          int ticket_activity_attachment_maxsize) {
  sqlite3_stmt *stmt = nullptr;
  const char *sql = R"SQL(
INSERT OR REPLACE INTO helpdesk_setting
  (name, default_status_id, default_assigned_to_id,
   ...
   ticket_attachment_maxsize, ticket_activity_attachment_maxsize)
VALUES ('default', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
)SQL";
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
  ...
  sqlite3_bind_int(stmt, 9, ticket_attachment_maxsize);
  sqlite3_bind_int(stmt, 10, ticket_activity_attachment_maxsize);
  const bool ok = sqlite3_step(stmt) == SQLITE_DONE;
  if (!ok)
...
void RenderSettingsConfig(const Callback &callback, sqlite3 *db) {
  ...
  data.insert("ticket_attachment_maxsize",
              setting ? setting->at("ticket_attachment_maxsize") : "47185920");
  data.insert("ticket_activity_attachment_maxsize",
              setting ? setting->at("ticket_activity_attachment_maxsize")
                      : "15728640");
  data.insert("statuses", statuses);
  data.insert("assignees", assignees);
  data.insert("priorities", priorities);
