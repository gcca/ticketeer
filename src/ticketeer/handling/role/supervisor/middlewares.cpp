...
namespace ticketeer::handling::role::supervisor::middlewares {

void RoleSupervisorRequired::invoke(const drogon::HttpRequestPtr &req,
...
SELECT p.id::text
 ...
 WHERE s.token = $1
  ...
  if (status != PGRES_TUPLES_OK)
    LOGJ_DEBUG(logger, "[supervisor] role middleware query error",
