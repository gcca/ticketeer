...
void LogInRequired::invoke(const drogon::HttpRequestPtr &req,
  ...
  if (!username) {
    LOGJ_DEBUG(logger,
               "[auth/middleware] invalid or expired token, deleting session");
    const bool signed_out = auth::utils::SignOut(logger, db, token);
    if (!signed_out)
      LOGJ_DEBUG(logger,
                 "[auth/middleware] session already deleted or not found");
    sqlite3_close(db);
