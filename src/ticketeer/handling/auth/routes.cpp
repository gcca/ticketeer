...
UpsertOAuthUser(sqlite3 *db, const std::string &username,
...
RETURNING id
  ...
  if (sqlite3_prepare_v2(db, upsert_sql, -1, &stmt, nullptr) != SQLITE_OK) {
    LOG_DEBUG << "[auth/app] upsert user error: " << sqlite3_errmsg(db);
    return std::nullopt;
  }
  ...
  if (!user_id) {
    LOG_DEBUG << "[auth/app] upsert returned no row";
    return std::nullopt;
  }
...
ON CONFLICT(user_id) DO NOTHING
  ...
  if (sqlite3_prepare_v2(db, profile_sql, -1, &stmt, nullptr) != SQLITE_OK) {
    LOG_DEBUG << "[auth/app] profile insert error: " << sqlite3_errmsg(db);
    return user_id;
  }
...
void Auth::SignInGet(const drogon::HttpRequestPtr &req, Callback &&callback) {
  ...
  if (not token.empty()) {
    ...
    if (db) {
      const auto username = handling::auth::utils::FetchUsername(db, token);
      sqlite3_close(db);
      if (username)
        return callback(drogon::HttpResponse::newRedirectionResponse(
            "/ticketeer/dashboard"));
    }
  }
...
void Auth::SignInPost(const drogon::HttpRequestPtr &req, Callback &&callback) {
  ...
  const auto user_id =
      handling::auth::utils::Authenticate(db, username, password);
  ...
  const bool ok = handling::auth::utils::LogIn(db, *user_id, resp);
  sqlite3_close(db);
...
void Auth::SignOutPost(const drogon::HttpRequestPtr &req, Callback &&callback) {
  ...
  const bool deleted = handling::auth::utils::SignOut(db, token);
  sqlite3_close(db);
...
void Auth::AppSignInGet(const drogon::HttpRequestPtr &, Callback &&callback) {
  ...
  if (!provider) {
    LOG_DEBUG << "[auth/app] no provider configured";
  ...
  client->sendRequest(
      msReq, [client, cb = std::move(callback)](
        ...
        if (res != drogon::ReqResult::Ok) {
          LOG_DEBUG << "[auth/app] device code request failed";
        ...
        if (json.isMember("error")) {
          LOG_DEBUG << "[auth/app] device code error: "
                    << json["error"].asString();
          auto resp = drogon::HttpResponse::newHttpResponse();
        ...
        if (user_code.empty() || device_code.empty()) {
          ...
          LOG_DEBUG << "[auth/app] empty user_code/device_code from MS: "
                    << ms_body;
          auto resp = drogon::HttpResponse::newHttpResponse();
          resp->setStatusCode(drogon::k502BadGateway);
  ...
  client->sendRequest(
      msReq, [client, cb = std::move(callback),
        ...
        if (!error.empty()) {
          ...
          LOG_DEBUG << "[auth/app] token error: " << error;
          Json::Value j;
          j["status"] = "error";
        ...
        const auto user_id = UpsertOAuthUser(db, username, display_name);
        if (!user_id) {
          sqlite3_close(db);
          Json::Value j;
          j["status"] = "error";
