...
TEST(AuthUtilsTest, AuthenticateReturnsUserIdForMatchingCredentials) {
  ...
  const auto user_id = Authenticate(db.get(), "alice", "password");
  ASSERT_TRUE(user_id);
  EXPECT_EQ(7U, *user_id);

  EXPECT_FALSE(Authenticate(db.get(), "alice", "wrong-password"));
  EXPECT_FALSE(Authenticate(db.get(), "missing", "password"));
}
...
TEST(AuthUtilsTest, LogInFetchUsernameAndSignOutManageSession) {
  ...
  ASSERT_TRUE(LogIn(db.get(), 7, resp));
  ...
  const auto username = FetchUsername(db.get(), cookie.value());
  ASSERT_TRUE(username);
  EXPECT_EQ("alice", *username);

  EXPECT_TRUE(SignOut(db.get(), cookie.value()));
  EXPECT_FALSE(FetchUsername(db.get(), cookie.value()));
}
...
TEST(AuthUtilsTest, FetchUsernameReturnsFalseForUnknownToken) {
  ...
  EXPECT_FALSE(FetchUsername(db.get(), "ticketeer-v1_nonexistent"));
}
