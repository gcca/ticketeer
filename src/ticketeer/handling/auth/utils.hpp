...
VerifyPassword(const std::string &password, const std::string &stored) {
  auto p1 = stored.find('$');
  ...
  if (hex_dk.size() != dklen * 2) return false;
  for (uint32_t i = 0; i < dklen; ++i) {
    char buf[3];
    std::snprintf(buf, sizeof(buf), "%02x", dk[i]);
    if (hex_dk[2 * i] != buf[0] || hex_dk[2 * i + 1] != buf[1]) return false;
  }
  return true;
}
...
[[nodiscard]] static inline bool
LogIn(quill::Logger *logger, sqlite3 *db, std::uint64_t user_id,
