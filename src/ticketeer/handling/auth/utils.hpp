...
Authenticate(sqlite3 *db, const std::string &username,
...
SELECT id, password
 ...
 WHERE username = ?
  ...
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
    LOG_DEBUG << "[auth] query error: " << sqlite3_errmsg(db);
    return std::nullopt;
  }
...
[[nodiscard]] static inline bool LogIn(sqlite3 *db, std::uint64_t user_id,
...
RETURNING token, expires_at
  ...
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
    LOG_DEBUG << "[auth] insert error: " << sqlite3_errmsg(db);
    return false;
  }
...
FetchUsername(sqlite3 *db, const std::string &token) {
  sqlite3_stmt *stmt = nullptr;
  const char *sql = R"SQL(
SELECT u.username
  FROM auth_session s
...
[[nodiscard]] static inline bool SignOut(sqlite3 *db,
                                         const std::string &token) {
  sqlite3_stmt *stmt = nullptr;
  const char *sql = "DELETE FROM auth_session WHERE token = ?";

  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
    LOG_DEBUG << "[auth] logout error: " << sqlite3_errmsg(db);
    return false;
  }
